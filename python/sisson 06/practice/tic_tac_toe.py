...
def check_replace(board, r, c):
    if board[r][c] != "_":
        ...
        return True
def enter_choice(player, board, char1, char2, g):
    if g == 2 and player == 2:
        while True:
            player_r = random.randint(1, 3)
...
def check_game(board, player1, player2, g=1):
    winer = ""
    for r in range(3):
        s1r = 0
        s2r = 0
...
while winer == "" and winer != "equal":
    show_board(board)
    enter_choice(1, board, player1_char, player2_char, game)
    winer = check_game(board, player1_char, player2_char, game)
    if winer != "":
        continue
    show_board(board)
    enter_choice(2, board, player1_char, player2_char, game)
    winer = check_game(board, player1_char, player2_char, game) 
show_board(board)
if winer == "equal":
    text = "equal"
